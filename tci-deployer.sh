...
TCI_SCRIBE_SUPERUSER_AUTH=${TCI_SCRIBE_SUPERUSER_AUTH:-'provisioning@tsc.com:550T1BC@'}
MS_COUNT=${MS_COUNT:-''}

# This variable is used in prefixed deployments
DEPLOYMENT_NAME=${DEPLOYMENT_NAME:-""}
TSC_ADMIN_AD_USERNAME=${TSC_ADMIN_AD_USERNAME:-""}
TSC_ADMIN_AD_PASSWORD=${TSC_ADMIN_AD_PASSWORD:-""}
FTL_SUPPORT=${FTL_SUPPORT:-""}
IMAGE_TAG='17808'
...
docker run ${MODE} --rm \
  ...
  -e FTL_SUPPORT=${FTL_SUPPORT} \
  -e TSC_ADMIN_AD_USERNAME=${TSC_ADMIN_AD_USERNAME} \
  -e TSC_ADMIN_AD_PASSWORD=${TSC_ADMIN_AD_PASSWORD} \
  -e CONTAINER_NAME=${CONTAINER_NAME} \
  -e DEPLOYER_DATA=${DEPLOYER_DATA} \
  -e TCI_SCRIBE_SUPERUSER_AUTH=${TCI_SCRIBE_SUPERUSER_AUTH} \
  -e MS_COUNT=${MS_COUNT} \
  ${TCI_MOUNT}\
  ${AZURE_CREDENTIALS}\
  ${AZURE_SETTINGS} \
